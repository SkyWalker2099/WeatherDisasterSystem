...
class BertWeatherClassifer(object):
    ...
    def generater(self,x_train1,x_train2,y_train,batch_size,mask_rate=0.1):
        while True:
            x1_train = x_train1[0]
            x2_train = x_train1[1]

            x11_train = x_train2[0]
            x22_train = x_train2[1]

            len1 = len(x1_train)
            len2 = len(y_train)
            if(len1 != len2 or len(x1_train) != len(x11_train) or len(x11_train) != len(x22_train)):
            ...
            while i*batch_size + batch_size < len1:
                ...
                x11_in = x11_train[i * batch_size:i * batch_size + batch_size]
                x11_in = self.mask_data(x11_in, mask_rate=mask_rate)

                x2_in = np.array(x2_train[i*batch_size:i*batch_size + batch_size])
                x22_in = np.array(x22_train[i * batch_size:i * batch_size + batch_size])


                y = y_train[i*batch_size:i*batch_size + batch_size]
                i+=1
                yield({'input_1':x1_in,'input_2':x2_in,'input_3':x11_in,'input_4':x22_in},{'dense_2':y})
...
if __name__ == '__main__':
     ...
     X1 = bwc.get_encode(newss)
     X2 = bwc.get_encode(titles)
     Y = lables2onehot(labels, 12)

     checkpoint = ModelCheckpoint('bert_masked_{epoch:03d}.h5', save_weights_only=True, period=1)
     history = bwc.model.fit_generator(generator=bwc.generater(X1,X2, Y, batch_size=1, mask_rate=0.1), epochs=5,
                                       steps_per_epoch=int(len(Y) / 1), callbacks=[checkpoint])
     bwc.model.save("MODELS\\bert_masked_12_title_news.h5")
     pickle.dump(history, open("训练记录/WC2/history2.pk", 'wb'))
