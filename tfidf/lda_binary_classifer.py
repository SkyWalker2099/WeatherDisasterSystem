...
lda_path = "MODELS\\lda\\lda.pk"
clf_path = "MODELS\\lda\\lda_svm_clf.pk"

classifications = ["disaster","not_disaster"]

class LdaClassifer(object):
    def __init__(self,lda_path,tfidf_path,clf_path):
        self.lda_path = lda_path
        self.tfidf_path = tfidf_path
        self.clf_path = clf_path
        self.tv = pickle.load(open(self.tfidf_path,'rb'))
        self.lda = pickle.load(open(self.lda_path,'rb'))
    ...
    def predict(self,data):
        ...
        data_lda = self.lda.transform(data_tfidf)
        results = self.clf.predict(data_lda)
...
lc = LdaClassifer(lda_path,tfidf_path,clf_path)
...
"""
    ...
    result1 = lc.predict(test_data)  #输入数据为pandas.core.series.Series类型
    print(result1)

    result2 = lc.predict(s)   #输入数据为list类型
    print(result2)

    result3 = lc.predict(s1)  #输入数据为string类型
    print(result3)
