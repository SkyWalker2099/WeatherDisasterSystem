...
class BertWeatherRecongizer(object):
    def build_bert(self):
        ...
        x3 = Dropout(0.5)(x2)
        p = Dense(2, activation='softmax')(x3)
    ...
    def generater(self,x_train,y_train,batch_size,mask_rate=0.1):
        while True:
            ...
            while i*batch_size + batch_size < len1:
                ...
                yield ({'input_1': x1_in, 'input_2': x2_in}, {'dense_2': y})
...
if __name__ == '__main__':
     ...
     train_data = pd.read_csv("D:\MyProject\天气灾害分类及信息提取\灾害新闻识别（二分）\DATA\超短句\strain.csv")
     test_data = pd.read_csv("D:\MyProject\天气灾害分类及信息提取\灾害新闻识别（二分）\DATA\超短句\stest.csv")
     ...
     titles2 = test_data["title"]
     newss2 = test_data["news"]
     labels2 = test_data["label"]

     Xs2 = [ str(t) + n for t, n in zip(titles2, newss2)]
     X2 = bwc.get_encode(Xs2)
     Y2 = lables2onehot(labels2, 2)

     #
     # checkpoint = ModelCheckpoint('MODELS\\short_model\\bert_short_{epoch:03d}.h5',save_weights_only=True,period=1)
     # history = bwc.model.fit_generator(generator=bwc.generater(X,Y,batch_size=2,mask_rate=0),validation_data=(X2,Y2),epochs=5,steps_per_epoch=int(len(Y)/2),callbacks=[checkpoint])
     # bwc.model.save("MODELS\\short_model\\bert_short.h5")
     # pickle.dump(history, open("MODELS\\short_model\\history.pk", 'wb'))

     #
     # bwc.model.load_weights("MODELS\\short_model\\bert_short_004.h5")
     # score = bwc.model.evaluate(X2,Y2)
     # print(score)
