...
class HierarchicalBertWeatherRecongizer(object):
    ...
    def build_bert(self,trainable = False):
    ...
    def predict_str2(self,title,news):
        swords = ["未来","预警"]
        for w in swords:
            if title.find(w) != -1:
                return 1
...
if __name__ == '__main__':
    ...
    hbc.build_lstm()
    hbc.lstm_model.load_weights("MODELS\\h_bert\\hbbc2.h5")
    ...
    ndatas = pd.read_csv("../DATA/SYN/CSV/test.csv")
    titles = ndatas["title"]
    newss = ndatas["news"]
    labels = ndatas["label"]
    count = 0
