...
class SynBertWeatherRecongizer(object):
    ...
    def predict_str(self,news):
        if len(news) > 500:
            return self.predict_long(news)
        else:
            return self.predict_short(news)
...
if __name__ == '__main__':
    ...
    pass
#训练短文本
    # train_data = pd.read_csv("DATA\\CSV\\strain.csv")
    ...
    # train_x, test_x, train_y, test_y = train_test_split(Xs, Y, random_state=2020, test_size=0.02)
    ...
    #
    #
    # sbwr.bert.fit_generator(validation_data=(test_x,test_y),generator=sbwr.generater(train_x,train_y,batch_size=2,mask_rate=0.1),epochs=5,steps_per_epoch=int(len(train_y)/2))
    #
    # sbwr.bert.save("MODELS\\syn_model\\bert_short.h5")

#
    sbwr.bert.load_weights("MODELS\\syn_model\\bert_short.h5")
    pass
#开始处理长文本生成npy
    # ldatas = pd.read_csv("DATA\\CSV\\ltrain.csv")
    # titles = ldatas["title"]
    # newss = ldatas["news"]
    # labels = ldatas["label"]
    #
...
#训练lstm
    ...
    #
    # train_x, test_x, train_y, test_y = train_test_split(X,Y,random_state=2020, test_size=0.02)
    #
    # print(train_x.shape)
    # print(train_y.shape)
    # print(test_x.shape)
    # print(test_y.shape)
    #
    # sbwr.lstm_model.fit(validation_data=(test_x,test_y),x = train_x,y = train_y,batch_size=32,epochs=4)
    #
    # sbwr.lstm_model.save("lstm_model.h5")
...
# 文本测试
    ...
    test_data = pd.read_csv("DATA\\CSV\\test.csv")
    ...
    #0.9570063694267515 0.9310611928737413 0.9490740740740741
#
