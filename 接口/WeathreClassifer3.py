...
classifications2 = ['地质灾害','城市内涝','暴雨洪涝']
...
class BertWeatherClassifer(object):
    ...
    def build_bert2(self):
        ...
        # p = Dense(12, activation='softmax')(x)
        self.model2 = Model([x1_in, x2_in], p)
        self.model2.compile(
            # loss = 'binary_crossentropy',
            loss='categorical_crossentropy',
            optimizer=Adam(1e-5),  # 用足够小的学习率
    ...
    def build_bert3(self):
        ...
        # p = Dense(12, activation='softmax')(x)
        self.model3 = Model([x1_in, x2_in], p)
        self.model3.compile(
            # loss = 'binary_crossentropy',
            loss='categorical_crossentropy',
            optimizer=Adam(1e-5),  # 用足够小的学习率
    ...
    def predict_str(self,x):
        ...
        if index in [3,4,8]:
            result = self.model2.predict(s)
            index = result.argmax(axis=1)
            index = index[0]
            index = ks[index]
        elif index in [0,10]:
            result = self.model3.predict(s)
            index = result.argmax(axis=1)
            index = index[0]
            index = ks2[index]
...
if __name__ == '__main__':
     ...
     bwc.model3.load_weights("temp\\bert_masked_diwen_xuezai010.h5")
     ...
     train_data = train_data[train_data["label"].isin(["低温灾害","雪灾灾害"])]
     ...
     #
     checkpoint = ModelCheckpoint('bert_masked_diwen_xuezai{epoch:03d}.h5',save_weights_only=True,period=1)
     history = bwc.model3.fit_generator(generator=bwc.generater(X,Y,batch_size=2,mask_rate=0.1),epochs=10,steps_per_epoch=int(len(Y)/2),callbacks=[checkpoint])
     # bwc.model.save("MODELS\\bert_masked_12_4.h5")
     pickle.dump(history, open("history2_diwen_xuezai.pk", 'wb'))
